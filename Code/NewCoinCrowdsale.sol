...
contract NewCoinSale is Crowdsale, MintedCrowdsale, CappedCrowdsale, TimedCrowdsale, RefundablePostDeliveryCrowdsale {
    ...
    constructor(
        ...
        uint goal, // target number (minumum) tokens to sell. if sales < goal, contract returns ETH to participants. Also using to set maximum amount of ETH to be raised in crowdsale
        uint open, // test: use fakenow
        uint close // test: use fakenow + 1 minutes
    
    )
        // Pass the constructor parameters to the crowdsale contracts.
        Crowdsale (rate,wallet,token)
        CappedCrowdsale (goal)
        // TimedCrowdsale (fakenow, now + 1 minutes) // test
        TimedCrowdsale (open, close) // production
...
contract NewCoinSaleDeployer {
        ...
        public
    {
        ...
        token_address = address(token); // fetch token address
        
        // Create the NewCoinSale and tell it about the token, set the goal, and set the open and close times to now and now + 24 weeks.
        //NewCoinSale token_sale = new NewCoinSale(1, wallet, token, goal, fakenow, now + 1 minutes); // test
        NewCoinSale token_sale = new NewCoinSale(1, wallet, token, goal, now, now + 24 weeks); // production
